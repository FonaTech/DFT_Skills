...
def compare_spine_sources(
    manifest_spine: dict[str, Any],
    markdown_spine: dict[str, str],
    strict: bool,
    warnings: list[str],
    errors: list[str],
    source_name: str = "Research spine",
) -> None:
    for field in ("objective", "active_claim_id", "current_gate", "next_action", "stop_rule"):
        ...
        if manifest_value and markdown_value and manifest_value != markdown_value:
            (errors if strict else warnings).append(
                f"{source_name} drift for {field}: manifest={manifest_value!r}, document={markdown_value!r}"
            )


def planning_artifact_path(project_root: Path, manifest: dict[str, Any], errors: list[str]) -> Path | None:
    artifact = manifest.get("planning_artifact")
    if not isinstance(artifact, dict):
        if manifest.get("schema_version") == "1.1":
            errors.append("schema 1.1 manifest is missing planning_artifact.")
        return None
    raw_path = artifact.get("path")
    if not isinstance(raw_path, str) or not raw_path.strip():
        errors.append("planning_artifact.path is missing.")
        return None
    candidate = Path(raw_path)
    if candidate.is_absolute():
        errors.append("planning_artifact.path must be project-relative.")
        return None
    resolved = (project_root / candidate).resolve()
    if resolved != project_root and project_root not in resolved.parents:
        errors.append("planning_artifact.path escapes the project root.")
        return None
    return resolved


def inspect_planning_artifact(
    path: Path | None,
    ...
    result = inspect_plan(
        ...
        require_current=strict,
        strict=strict,
    )
    errors.extend(f"Planning artifact: {message}" for message in result["errors"])
    warnings.extend(f"Planning artifact: {message}" for message in result["warnings"])
    artifact = manifest.get("planning_artifact")
    declared_status = artifact.get("status") if isinstance(artifact, dict) else None
    document_status = result.get("status")
    if isinstance(declared_status, str) and isinstance(document_status, str) and declared_status != document_status:
        (errors if strict else warnings).append(
            f"Planning artifact status drift: manifest={declared_status!r}, document={document_status!r}"
        )
    return result
...
def markdown_report(report: dict[str, Any]) -> str:
    ...
    planning_artifact = report.get("planning_artifact") or {}
    planning_status = planning_artifact.get("status", "<not tracked>") if isinstance(planning_artifact, dict) else "<not tracked>"
    planning_path = planning_artifact.get("path", "<not tracked>") if isinstance(planning_artifact, dict) else "<not tracked>"
    lines = [
        "# Research Status",
        "",
        ...
        f"- Integrated plan: {planning_path} ({planning_status})",
        "",
        "## Counts",
        "",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--plan-file", type=Path, help="Override workflow/integrated_research_plan.md path.")
...
def main() -> int:
    ...
    declared_plan_path = planning_artifact_path(root, manifest, errors)
    plan_path = (args.plan_file.expanduser().resolve() if args.plan_file else declared_plan_path)
    ...
    planning_artifact = inspect_planning_artifact(plan_path, manifest, args.strict, warnings, errors)
    ...
    if isinstance(manifest_spine, dict):
        ...
        if isinstance(planning_artifact, dict):
            snapshot = planning_artifact.get("snapshot")
            if isinstance(snapshot, dict):
                compare_spine_sources(
                    manifest_spine,
                    snapshot,
                    args.strict,
                    warnings,
                    errors,
                    source_name="Planning artifact control snapshot",
                )
    report["spine_file"] = str(spine_path)
    report["spine_markdown"] = markdown_spine
    report["planning_artifact_path"] = str(plan_path) if plan_path else None
    report["planning_artifact"] = planning_artifact
    ...
    if args.pretty:
        ...
        plan_status = (report.get("planning_artifact") or {}).get("status", "<not tracked>")
